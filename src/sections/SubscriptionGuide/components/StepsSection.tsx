import { useEffect, useState } from "react";

const TIER_STORAGE_KEY = "lust-playhouse-selected-tier";

type TierOption = {
  id: string;
  name: string;
  price: string;
  perk: string;
  features: string[];
  amount: number;
  highlight?: boolean;
};

const steps = [
  { id: "1", icon: "💳", copy: "Pick a tier & pay securely" },
  { id: "2", icon: "🔗", copy: "Redirect to secure checkout instantly" },
  { id: "3", icon: "🚀", copy: "Get instant access after payment" },
];

const tiers: TierOption[] = [
  {
    id: "tease-15",
    name: "Tease",
    price: "$15",
    amount: 1500,
    perk: "Starter access",
    features: ["1000+ Videos", "Bop Content", "Leaks"],
  },
  {
    id: "desire-25",
    name: "Desire",
    price: "$25",
    amount: 2500,
    perk: "Expanded access",
    features: ["3000+ Videos", "Higher Quality", "Everything in Tease"],
  },
  {
    id: "obsession-50",
    name: "Obsession",
    price: "$50",
    amount: 5000,
    perk: "Full access",
    features: ["5000+ Videos", "All Categories", "Priority Updates"],
    highlight: true,
  },
];

/* ================= COMMUNITY SECTION ================= */
const CommunityButtons = () => {
  return (
    <div className="mt-20 flex justify-center">
      <div className="w-full max-w-[520px] rounded-3xl border border-red-500/20 bg-gradient-to-b from-black/90 to-black/80 backdrop-blur-xl shadow-[0_0_80px_rgba(220,38,38,0.15)] p-6 md:p-8">

        <div className="flex justify-center mb-6">
          <div className="px-4 py-1 rounded-full border border-red-500/30 bg-red-500/10 text-red-300 text-xs tracking-[2px] uppercase">
            LUST PLAYHOUSE: LIVE
          </div>
        </div>

        <h2 className="text-2xl md:text-3xl font-black text-white text-center">
          Join The Community
        </h2>

        <p className="text-center text-gray-400 text-sm mt-2">
          Free channel, live chatroom & direct admin support
        </p>

        <div className="mt-6 space-y-4">

          {[
            {
              icon: "📡",
              title: "Join Free Channel",
              desc: "Previews & drops",
              link: "https://t.me/+FZv49DSqQ_lmODcx",
            },
            {
              icon: "💬",
              title: "Official Chatroom",
              desc: "Talk with members",
              link: "https://t.me/+KsCdMv3mCSVlY2Vh",
            },
            {
              icon: "🛡️",
              title: "Contact Admin",
              desc: "Support & orders",
              link: "https://t.me/savslayr",
            },
          ].map((item) => (
            <a
              key={item.title}
              href={item.link}
              target="_blank"
              rel="noreferrer"
              className="group flex items-center justify-between rounded-2xl border border-red-500/10 bg-black/40 p-4 hover:border-red-400/50 hover:bg-black/60 transition"
            >
              <div className="flex items-center gap-3">
                <span className="text-lg">{item.icon}</span>
                <div>
                  <div className="text-white font-semibold group-hover:text-red-300">
                    {item.title}
                  </div>
                  <div className="text-xs text-gray-400">{item.desc}</div>
                </div>
              </div>
              <span className="text-red-300 group-hover:translate-x-1 transition">→</span>
            </a>
          ))}

        </div>

        <div className="mt-8 text-center text-[10px] tracking-[2px] text-gray-500 uppercase">
          © 2026 Lust Playhouse • Private Member Lounge
        </div>

      </div>
    </div>
  );
};

/* ================= MAIN ================= */
export const StepsSection = () => {
  const [selectedTier, setSelectedTier] = useState<TierOption>(tiers[0]);

  useEffect(() => {
    const stored = window.localStorage.getItem(TIER_STORAGE_KEY);
    if (stored) {
      const match = tiers.find((t) => t.id === stored);
      if (match) setSelectedTier(match);
    }
  }, []);

  const handleBuy = async (tier: TierOption) => {
    try {
      window.localStorage.setItem(TIER_STORAGE_KEY, tier.id);

      const res = await fetch("https://lustplayhouse.cloud/create-payment-link", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ amountCents: tier.amount }),
      });

      const data = await res.json();

      if (!res.ok) {
        alert("Payment error: " + JSON.stringify(data));
        return;
      }

      if (data.url) {
        window.location.href = data.url;
      }
    } catch (err) {
      console.error(err);
      alert("Server error. Try again.");
    }
  };

  return (
    <div className="w-full mx-auto max-w-[1000px] px-4 text-center">

      {/* STEPS */}
      <div className="grid grid-cols-3 gap-5 mb-12">
        {steps.map((step) => (
          <div
            key={step.id}
            className="rounded-2xl border border-red-500/10 bg-gradient-to-b from-black/80 to-black/60 backdrop-blur-xl p-6 shadow-[0_0_40px_rgba(220,38,38,0.08)] hover:border-red-400/30 transition"
          >
            <div className="text-2xl mb-3">{step.icon}</div>
            <div className="text-sm text-white/90">{step.copy}</div>
          </div>
        ))}
      </div>

      <h2 className="text-3xl md:text-4xl font-black text-white">
        Choose Your Tier
      </h2>

      <p className="mt-2 text-gray-400">
        One-time payment · lifetime access · instant delivery
      </p>

      {/* TIERS */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12 items-stretch">

        {tiers.map((tier) => (
          <div
            key={tier.id}
            className={`relative flex flex-col justify-between min-h-[480px] rounded-3xl p-7 text-left transition duration-300
              ${
                tier.highlight
                  ? "border border-red-500 bg-gradient-to-b from-[#120606] to-black shadow-[0_0_90px_rgba(239,68,68,0.35)] scale-[1.03]"
                  : "border border-white/10 bg-black/60 hover:border-red-400/30"
              }`}
          >

            {tier.highlight && (
              <span className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-red-500 text-white text-[10px] font-bold uppercase tracking-widest shadow-[0_0_20px_rgba(239,68,68,0.6)]">
                ★ Highest Tier
              </span>
            )}

            {/* TOP */}
            <div>

              <p className="text-sm font-semibold text-red-300 uppercase tracking-widest">
                {tier.name}
              </p>

              <div className="mt-3 flex items-end gap-2">
                <span className="text-5xl font-black text-red-400">
                  {tier.price}
                </span>
                <span className="text-xs text-gray-500 pb-2">
                  one-time
                </span>
              </div>

              <div className="mt-8 space-y-3 text-sm text-gray-300">
                {tier.features.map((f) => (
                  <div key={f} className="flex items-center gap-2">
                    <span className="text-red-400">✓</span>
                    <span>{f}</span>
                  </div>
                ))}
              </div>

            </div>

            {/* BUTTON */}
            <div className="mt-10">
              <button
                onClick={() => handleBuy(tier)}
                className={`w-full py-3 rounded-full font-bold transition shadow-lg
                  ${
                    tier.highlight
                      ? "bg-gradient-to-r from-red-600 to-red-400 text-white hover:opacity-90"
                      : "bg-red-500 text-black hover:bg-red-400"
                  }`}
              >
                Get {tier.name} — {tier.price}
              </button>
            </div>

          </div>
        ))}
      </div>

      <CommunityButtons />
    </div>
  );
};
